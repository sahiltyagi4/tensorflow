...
def get_model_fn(num_gpus, variable_strategy, num_workers, gradient_scale):
  ...
  def _resnet_model_fn(features, labels, mode, params):
      ...
      with tf.variable_scope('resnet', reuse=bool(i != 0)):
        with tf.name_scope('tower_%d' % i) as name_scope:
          with tf.device(device_setter):
            #tf.logging.info('@@@@@@@ tower function device name')
            #tf.logging.info(device_setter)
    ...
    with tf.device(consolidation_device):
      #tf.logging.info('!!!!!!!!!loss/optimizer consolidation device')
      #tf.logging.info(consolidation_device)
      ...
      #tf.logging.info('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ length of gradvars list ' + str(len(gradvars)))
      train_op = [
...
def _tower_fn(is_training, weight_decay, feature, label, data_format,
  ...
  #tf.logging.info('done reading ORIGINAL gradient....')

  # tower_grad *= (40 / 80)
  # for grad in tower_grad:
  #   tf.logging.info('modified grad value is %f' % grad)

  # tf.logging.info('done reading MODIFIED gradient....')


  compgrad_tower = optimizer.compute_gradients(tower_loss, model_params)
  for g,v in compgrad_tower:
    tf.summary.histogram(v.name + '_ORG', g)
  
  new_grads = [(grad[0] * gradient_scale) for grad in compgrad_tower]
  for new_g, new_v in zip(new_grads, model_params):
    tf.summary.histogram(new_v.name + '_MODIFIED', new_g)

  #return tower_loss, zip(tower_grad, model_params), tower_pred
  #return tower_loss, compgrad_tower, tower_pred
  # when running for actual per-node batch-size gradient scaling, comment the above and uncomment whats below..
  return tower_loss, zip(new_grads, model_params), tower_pred
...
def main(job_dir, data_dir, num_gpus, variable_strategy,
  ...
  if tasktype == 'worker':
    if index == 0:
      node_batch_size = batchlist[2]
    if index == 1:
      node_batch_size = batchlist[3]

  # run_config = cifar10_utils.RunConfig(
  #     session_config=sess_config, model_dir=job_dir)
  run_config = tf.estimator.RunConfig(session_config=sess_config, model_dir=job_dir, node_batch_size=node_batch_size, save_checkpoints_steps=990)
  ...
  average_batchsize = (batchsum/(len(batchlist) -1))
