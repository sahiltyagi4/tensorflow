...
def _tower_fn(is_training, weight_decay, feature, label, data_format,
  ...
  new_grads = [(grad[0] * 0.5) for grad in compgrad_tower]
  ...
  #return tower_loss, zip(only_grads, model_params), tower_pred
...
def main(job_dir, data_dir, num_gpus, variable_strategy,
  ...
  run_config = tf.estimator.RunConfig(session_config=sess_config, model_dir=job_dir, node_batch_size=64)
  ...
  acc_threshold = tf.estimator.experimental.stop_if_higher_hook(estimator=classifier, metric_name="accuracy", threshold=0.9074)
