...
def get_model_fn(num_gpus, variable_strategy, num_workers):
  ...
  def _resnet_model_fn(features, labels, mode, params):
    ...
    with tf.device(consolidation_device):
      ...
      tensors_to_log = {'learning_rate': learning_rate, 'loss': loss, 'accuracy': accuracy[1]}
...
def get_experiment_fn(data_dir,
  ...
  def _experiment_fn(run_config, hparams):
    ...
    acc_threshold = tf.estimator.experimental.stop_if_higher_hook(estimator=classifier, metric_name="accuracy", threshold=0.5)
    # Create experiment.
